  ...
  property("Use more payments") {
    ...
    val scenario =
      for {
        ...
        payments = List(
          Payment(10L, Waves),
          Payment(1L, IssuedAsset(iTx1.id())),
          Payment(3L, IssuedAsset(iTx3.id())),
          Payment(2L, IssuedAsset(iTx2.id()))
        )
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V3, invoker, master.toAddress, Some(fc), payments, fee, Waves, ts + 6)
          .explicitGet()
      ...
      val expr = {
        val script =
          s"""
             ...
             |  if b1 == b1 && ob1 == ob1
             |  then
             |    let r = Invoke(Alias("${alias.name}"), "bar", [this.bytes], [AttachedPayment(unit, 17)${assets
               .map(a => ", AttachedPayment(base58'" ++ a ++ "', 1)")
               .mkString("")}])
             |    if r == 17
             |    then
             |     let data = getIntegerValue(Address(base58'$otherAcc'), "bar")
